...
def verificar_acesso(current_user_id):
    ...
    try:
        ...
        if not funcionalidade:
            return jsonify({'success': False, 'error': 'funcionalidade é obrigatória'}), 400
        
        tem_acesso = plano_service.verificar_acesso(current_user_id, funcionalidade)
        return jsonify({
            'success': True,
            'data': {'tem_acesso': tem_acesso}
        }), 200
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def processar_pagamento(current_user_id):
    ...
    try:
        ...
        if not plano_id or not metodo_pagamento:
            return jsonify({'success': False, 'error': 'plano_id e metodo_pagamento são obrigatórios'}), 400
